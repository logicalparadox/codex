...
var CodexError = require('tea-error')('CodexError')
...
Project.prototype.parseMarkdown = function (text) {
  marked.setOptions({
    ...
    , highlight: function (code, lang) {
        ...
        return highlight.LANGUAGES.hasOwnProperty(lang)
...
function assertFolders (cb) {
  ...
  fs.exists(cfg.inDir, function (exists) {
    if (!exists) return cb(new CodexError('Input directory does not exist.'));
    ...
    fs.exists(cfg.templateDir, function (texist) {
      if (!texist) return cb(new CodexError('Template directory does not exist.'));
      clean(function (err) {
...
function loadPlugins (next) {
  ...
  async.forEachSeries(stack, function (plugin, cb) {
    // try to load plugin
    if (plugins[plugin.name]) {
      ...
      try { var mw = require(plugin.name); }
        ...
        return cb(new CodexError('Template \'' + plugin.name + '\' does not exist.'));
      }
    }
...
function renderFiles (cb) {
  ...
  async.forEach(this.files, function (file, done) {
