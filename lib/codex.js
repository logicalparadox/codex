...
tea.merge(Codex.prototype, drip.prototype);
...
Codex.prototype.build = function () {
  ...
  tea.mkdir(outDir, 0755, function (err) {
    if (err) throw err;
    ...
    if (path.existsSync(cfgPath)) {
      ...
      if (cfg.code) {
        for (var name in cfg.code) {
          var file = path.resolve(dataDir, cfg.code[name])
    ...
    this._getMDFiles(dataDir, function(err, results) {
      ...
      results.forEach(function (res) {
        ...
        if (yml) {
          ...
          defaults = tea.defaults(props, defaults);
        }
        ...
        result = tea.merge(defaults, result);
...
Codex.prototype._renderJade = function (locals) {
  ...
  jade.render(source, locals, function (err, html) {
    ...
    tea.mkdir(path.dirname(locals.file.outPath), 0755, function (err) {
      if (err) throw err;
...
Codex.prototype._prepareAssets = function () {
  ...
  tea.mkdir(path.join(assetOut, 'css'), 0755, function (err) {
    if (err) throw err;
